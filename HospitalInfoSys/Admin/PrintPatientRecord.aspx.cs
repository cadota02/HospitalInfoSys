...
{
    public partial class PrintPatientRecord : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (Request.QueryString["prid"] != null)
                {
                    ...
                    DataTable userData = GetUserDataFromDatabase(prid);
        ...
        protected void btnGenerateReport_Click(object sender, EventArgs e)
        {
            ...
            DataTable userData = GetUserDataFromDatabase("1");

            // Step 2: Generate the PDF report
            GeneratePDFReport(userData,"1");
        }

        private DataTable GetUserDataFromDatabase(string PRID)
        {
          
            using (MySqlConnection conn = new MySqlConnection(connString))
            {
                conn.Open();
                string query = "SELECT * FROM vw_patientrecord where PRID=@ID";
        ...
        private void GeneratePDFReport(DataTable userData,string DBID)
        {
            using (MemoryStream ms = new MemoryStream())
            {
                ...
                string filePath = Server.MapPath("~/reports/pdf/patient/" + fileName);

                File.WriteAllBytes(filePath, ms.ToArray());

                string filePath2 = ResolveUrl("~/reports/pdf/patient/" + fileName);
                pdfIframe.Attributes["src"] = filePath2;
        ...
        private void AddHeader(Document document)
        {
            ...
            Paragraph title = new Paragraph("Patient Record", titleFont);
        ...
        private void AddBody(Document document, DataTable userData)
        {
            ...
            {
                patientProfileTable.AddCell(new Phrase("Health No", cellFont));
                patientProfileTable.AddCell(new Phrase(row["HEALTHNO"].ToString(), cellFontHeader));
                ...
                patientProfileTable.AddCell(new Phrase(row["ADDRESS"].ToString() + ", " + row["CONTACTNO"].ToString(), cellFont));
                patientProfileTable.AddCell(new Phrase("Occupation", cellFontNine));
                patientProfileTable.AddCell(new Phrase(row["OCCUPATION"].ToString(), cellFont));
                patientProfileTable.AddCell(new Phrase("Emergency Contact", cellFontNine));
                patientProfileTable.AddCell(new Phrase(row["CPNAME"].ToString() + " - " + row["CPCONTACTNO"].ToString(), cellFont));
            }
            ...
            document.Add(new Paragraph("Patient Profile:", titleFont));
            ...
            foreach (DataRow row in userData.Rows)
            {
                medicalRecordTable.AddCell("Date Visit: " + row["PATLOGDATE"].ToString());

                medicalRecordTable.AddCell("Type/Room: " + row["TYPECONSULTATION"].ToString() + " (" + row["RoomName"].ToString() + " - " + row["RoomNumber"].ToString()+ ")");
                medicalRecordTable.AddCell("Complaint: " + row["CHIEFCOMPLAINT"].ToString());
                medicalRecordTable.AddCell("Medical History: " + row["MEDICALHISTORY"].ToString());
                medicalRecordTable.AddCell("Allergies: " + row["ALLERGIES"].ToString());
                medicalRecordTable.AddCell("Diagnosis: " + row["DIAGNOSIS"].ToString());
                medicalRecordTable.AddCell("Discharge Date: " + row["PATDISCHARGE"].ToString());
                medicalRecordTable.AddCell("Disposition: " + row["PATDISPOSITION"].ToString());

                doctor = new Paragraph(row["DoctorFullName"].ToString(), titleFontU);
                doctor.Alignment = Element.ALIGN_LEFT;
                doctor.SpacingBefore = 40f;
            }

            // Add the medical record table to the document
            document.Add(new Paragraph("\n Medical Records:", titleFont));
